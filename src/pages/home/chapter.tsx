...
import { ChevronLeftOutlined, CloseOutlined, PauseCircleFilled, PauseCircleOutlined } from '@mui/icons-material';
...
const HomeChapter: React.FC = () => {
  ...
  const [chapter, setChapter] = React.useState<ChapterSpider>({
    ...
    React.useEffect(() => {
      ...
      axios.get(`/spider/chapter?url=${url}`).then((res) => setChapter(res.data.data))
    }, [params])
  return (
    <>
